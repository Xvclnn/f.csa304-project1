...
def derivatives(v, h, t, m, t_deploy, constant_density):
...
def simulate_jump(m=85.0, h0=4000.0, t_deploy=60.0, dt=0.1, method='euler', constant_density=False):
    ...
    for i in range(max_steps - 1):
        ...
        if method == 'euler':
            dv, dh = derivatives(v[i], h[i], t[i], m, t_deploy, constant_density)
            v[i+1] = v[i] + dv * dt
            h[i+1] = h[i] + dh * dt
            
        elif method == 'rk4':
            dv1, dh1 = derivatives(v[i], h[i], t[i], m, t_deploy, constant_density)
            
            v_half1 = v[i] + 0.5 * dv1 * dt
            h_half1 = h[i] + 0.5 * dh1 * dt
            dv2, dh2 = derivatives(v_half1, h_half1, t[i] + 0.5 * dt, m, t_deploy, constant_density)
            
            v_half2 = v[i] + 0.5 * dv2 * dt
            h_half2 = h[i] + 0.5 * dh2 * dt
            dv3, dh3 = derivatives(v_half2, h_half2, t[i] + 0.5 * dt, m, t_deploy, constant_density)
            
            v_full = v[i] + dv3 * dt
            h_full = h[i] + dh3 * dt
            dv4, dh4 = derivatives(v_full, h_full, t[i] + dt, m, t_deploy, constant_density)
